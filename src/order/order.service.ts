  ...
  async createOrder(createOrderDto: CreateOrderDto, req, res) {
    const CustomerData = {
      customer: {
        ...
        password: "stylealoud",
        password_confirmation: "stylealoud",
    ...
    try {
      ...
      const customerResponse: AxiosResponse = await this.axiosInstance.get(`/customers/search.json?query=email:${createOrderDto.customer_email}`);
      let customerData;
      if (customerResponse.data.customers.length > 0) {
        // Customer already exists
        customerData = customerResponse.data.customers[0];
        console.log('Customer already exists:', customerData);
      } else {
      ...
      // Use customer data for the order
      const newOrder = {
        order: {
